...
###########################
    ...
    def get_gender_count(self, gender):
        self.c.execute(
            f"select count() from {self.survey_table} where gender={gender}")
        return int(self.c.fetchone()[0])

    def get_ethnicity_count(self, ethnicity):
        self.c.execute(
            f"select count() from {self.survey_table} where ethnicity={ethnicity}")
        return int(self.c.fetchone()[0])

    def get_disabled_count(self, disabled):
        self.c.execute(
            f"select count() from {self.survey_table} where disabled={disabled}")
        return int(self.c.fetchone()[0])

    def get_enjoyed_count(self, enjoyed):
        self.c.execute(
            f"select count() from {self.survey_table} where enjoyed={enjoyed}")
        return int(self.c.fetchone()[0])

    def get_curious_count(self, curious):
        self.c.execute(
            f"select count() from {self.survey_table} where curious={curious}")
        return int(self.c.fetchone()[0])

    def get_science_count(self, science):
        self.c.execute(
            f"select count() from {self.survey_table} where science={science}")
        return int(self.c.fetchone()[0])
...
###########################
    ...
    def set_settings(self, key, value):
        ...
        self.c.execute(
            f'''INSERT OR REPLACE INTO {self.settings_table} (key, value) VALUES("{key}", "{value}");''')
        self.conn.commit()

    def get_settings(self, key):
        self.c.execute(
            f"select (value) from {self.settings_table} where key='{key}'")
        try:
